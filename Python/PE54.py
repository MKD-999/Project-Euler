...
tri = ['KC AS 2D KH 9H 2C 5S 4D 3D 6H']
for i in hands:
	winner = None
	p1 = i[:15]
	p2 = i[15:]
	...
	if winner is None:
		...
		if set(p1_cards) == set('TJQKA'):
			if winner is None:
				winner = p1

		elif set(p2_cards) == set('TJQKA'):
			if winner is None:
				winner = p2

		if winner is not None:
			if winner == p1:
				myans.append('Player 1')
			elif winner == p2:
				myans.append('Player 2')

	# Straight Flush
	if winner is None:
		position = 'Straight Flush'
		p1_values = []
		for j in p1_cards:
			p1_values.append(ranks[j])
		p1_values.sort()

		p2_values = []
		for j in p2_cards:
			p2_values.append(ranks[j])
		p2_values.sort()
		...
		sf_p1_trial = []
		for s in p1_suits:
			if s not in sf_p1_trial:
				sf_p1_trial.append(s)

		if len(sf_p1_trial) == 1:
			if list(range(p1_values[0], p1_values[-1])) == p1_values:
				if winner is None:
					winner = p1

		sf_p2_trial = []
		for s in p2_suits:
			if s not in sf_p2_trial:
				sf_p2_trial.append(s)

		if len(sf_p2_trial) == 1:
			if list(range(p2_values[0], p2_values[-1])) == p2_values:
				if winner is None:
					winner = p2
				else:
					k = 0
					while True:
						if p1_values[k] > p2_values[k]:
							winner = p1
							break
						elif p1_values[k] < p2_values[k]:
							winner = p2
							break
						elif p1_values[k] == p2_values[k]:
							k += 1
		...
		for j in p2_cards:
			if p2_cards.count(j) == 4:
				...
				if winner is None:
					...
					if fk_p1 != '':
						if fk_p2 != '':
							if ranks[fk_p1] > ranks[fk_p2]:
								winner = fk_p1
							elif ranks[fk_p1] < ranks[fk_p2]:
								winner = fk_p2
							elif ranks[fk_p1] == ranks[fk_p2]:
								k = 0
								while True:
	...
	if winner is None:
		...
		p1_tochk = ''
		for j in p1_cards:
			if p1_cards.count(j) == 3:
				p1_tochk = j

		for j in p1_cards:
			if j != p1_tochk:
				if p1_tochk != '':
					if p1_cards.count(j) == 2:
						if winner is None:
							winner = p1

		p2_tochk = ''
		for j in p2_cards:
			if p2_cards.count(j) == 3:
				p2_tochk = j

		for j in p2_cards:
			if j != p2_tochk:
				if p2_tochk != '':
					if p2_cards.count(j) == 2:
						if winner is None:
							winner = p2
						else:
							if p1_tochk != '':
								if p2_tochk != '':
									if ranks[p1_tochk] > ranks[p2_tochk]:
										winner = p1
									elif ranks[p1_tochk] > ranks[p2_tochk]:
										winner = p2
									elif ranks[p1_tochk] == ranks[p2_tochk]:
										k = 0
										while True:
	...
	if winner is None:
		position = 'Flush'
		if len(set(p1_suits)) == 1:
			if winner is None:
				winner = p1

		if len(set(p2_suits)) == 1:
			if winner is None:
				winner = p2
			else:
				k = 0
	...
	if winner is None:
		...
		if list(range(p1_values[0], p1_values[-1]+1)) == p1_values:
			if winner is None:
				winner = p1

		elif list(range(p2_values[0], p2_values[-1]+1)) == p2_values:
			if winner is None:
				winner = p2
	...
	if winner is None:
		...
		if t_p2 != '':
			if winner is None:
				...
				if t_p1 != '':
					if ranks[t_p1] > ranks[t_p2]:
						winner = p1
					elif ranks[t_p1] > ranks[t_p2]:
						winner = p2
					elif ranks[t_p1] == ranks[t_p2]:
						k = 0
						while True:
	...
	if winner is None:
		...
		if len(p2_tot_cnt) == 2:
			if winner is None:
				...
				if max(p1_2p) > max(p2_2p):
					winner = p1
				elif max(p1_2p) < max(p2_2p):
					winner = p2
				elif max(p1_2p) == max(p2_2p):
					k = 0
					while True:
	...
	if winner is None:
		...
		if p2_op_tochk != '':
			if winner is None:
				...
				if p1_op_tochk != '':
					if p2_op_tochk != '':
						if ranks[p1_op_tochk] > ranks[p2_op_tochk]:
							winner = p1
						elif ranks[p1_op_tochk] < ranks[p2_op_tochk]:
							winner = p2
						elif ranks[p1_op_tochk] == ranks[p2_op_tochk]:
							k = 0
							while True:
